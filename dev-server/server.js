...
const home = router.get("/", function(req, res, next) {
  ...
  globals.template_content = engine.renderFile(`${views}/home.liquid`, globals);
  res.render("index", globals);
});

const posts = router.get("/posts", function(req, res, next) {
  globals.template.permalink = "posts";
  globals.template_content = engine.renderFile(
    `${views}/posts.liquid`,
    globals
  );
  res.render("index", globals);
...
const post = router.get("/posts/:article", function(req, res, next) {
  ...
  globals.template_content = engine.renderFile(`${views}/post.liquid`, globals);
  res.render("index", globals);
});
...
const facets = router.get("/search*", function(req, res, next) {
  if (secure) {
    request(
      `https://cm-preview-site-378510.ttcportals.com/${req.originalUrl}`
    ).pipe(res);
  } else {
    request(
      `https://cm-preview-site-378510.ttcportals.com/${req.originalUrl}`
...
const jobs = router.get("/job/:jobs", function(req, res, next) {
  ...
  globals.template_content = engine.renderFile(`${views}/job.liquid`, globals);
  res.render("index", globals);
});

const pages = router.get("/pages/:pagename", function(req, res, next) {
  globals.template.permalink = req.params.pagename;
  if (fs.existsSync(`${views}/pages/${req.params.pagename}.html`)) {
    globals.template_content = engine.renderFile(
      `${views}/pages/${req.params.pagename}`,
      globals
    );
  } else {
    const target = routeMap(req.params.pagename).path;
    const tags = routeMap(req.params.pagename).tags;
    globals.page.tags = tags;
    if (target) {
      globals.template_content = engine.renderFile(
        `${views}/pages${target}/${req.params.pagename}`,
        globals
      );
    } else {
      globals.template_content = engine.renderFile(
